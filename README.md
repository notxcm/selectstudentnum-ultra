...
| 🎁 **Surprise 惊喜** | 填入幸运数字 + 勾选「必选」，抽取时优先命中；配置永久保存，抽中出榜、重置恢复 |
...
- **打勾**：每次点抽号，优先**命中**榜内数字（按顺序，仍在奖池中的才抽，抽中即出榜），之后照常随机；
- **不打勾**：完全正常抽取，填入的数字会保留，随时可勾选启用。

必抽榜**配置永久保存**（重启不丢失）。抽中的数字会从当前活跃列表出榜（本次会话不再优先），点「重置奖池」后完整必抽榜恢复。
